...
int **dac_square_mat_product_parra(int **A, int **B, int n, int deallocate)
{
    ...
    else
    {
        ...
        #pragma omp parallel sections
        {
            #pragma omp section
            {
                C11 = add(dac_square_mat_product_parra(A11, B11, half_n, 0), dac_square_mat_product_parra(A12, B21, half_n, 0), half_n, 1+2);
            }
            #pragma omp section
            {
                C12 = add(dac_square_mat_product_parra(A11, B12, half_n, 1), dac_square_mat_product_parra(A12, B22, half_n, 1), half_n, 1+2);
            }
            #pragma omp section
            {
                C21 = add(dac_square_mat_product_parra(A21, B11, half_n, 2), dac_square_mat_product_parra(A22, B21, half_n, 2), half_n, 1+2);
            }
            #pragma omp section
            {
                C22 = add(dac_square_mat_product_parra(A21, B12, half_n, 1+2), dac_square_mat_product_parra(A22, B22, half_n, 1+2), half_n, 1+2);
            }
        }
